...
vec3 castRay( vec3 ro, vec3 rd, float linesSpeed )
{
    ...
    for( int i=0; i<128; i++ )
    {
        ...
        vec3 lcol = 0.6 + 0.4*sin( 10.0*6.2831*h + vec3(0.0,0.7,0.9) );
...
void main()
{
    ...
    float isFast = smoothstep( 35.8, 35.81, iGlobalTime );
    isFast  -= smoothstep( 61.8, 61.81, iGlobalTime );
    isFast  += smoothstep( 78.0, 78.01, iGlobalTime );
    isFast  -= smoothstep(103.0,103.01, iGlobalTime );
    isFast  += smoothstep(140.0,140.01, iGlobalTime );
    isFast  -= smoothstep(204.0,204.01, iGlobalTime );
